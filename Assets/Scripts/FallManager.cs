...
public class FallManager : MonoBehaviour {
	...
	public float lowerSpawnTimeBound = 2.5f;
	public float upperSpawnTimeBound = 2.7f;
	...
	public IEnumerator Fall() {
		while (true) { 
			...
			teleportHere = fallingCoordinate (gameObject.transform.position.y);
			Instantiate (fallingThing, teleportHere, Quaternion.identity);
			time = createRandomTime (lowerSpawnTimeBound, upperSpawnTimeBound); 
	...
	public Vector3 fallingCoordinate(float yValue) { 
		...
		Random.seed = System.DateTime.Now.Millisecond; //required, else same value keeps getting picked
		randomX =  Random.Range (0.9f, 0.95f);
		...
		coordinate = new Vector3 (gameObject.transform.position.x, coordinate.y, coordinate.z);
