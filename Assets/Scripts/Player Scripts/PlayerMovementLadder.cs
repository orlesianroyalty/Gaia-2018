...
public class PlayerMovementLadder : MonoBehaviour {
	...
	private GameObject touchedLadder;
	...
	void Update () {
		...
		inputVector = new Vector3 (horizontalInputFactor, 0.0f, verticalInputFactor);
	...
	void LadderCheck() {
		if (isOnLadder) {
			rb.useGravity = false;
			Vector3 touchedLadderPosition = touchedLadder.transform.position;
			float diff = touchedLadderPosition.x - transform.position.x;
			if ((diff > 0 && inputVector.x > 0) || (diff < 0 && inputVector.x < 0)) {
				inputVector.y = System.Math.Abs (inputVector.x);
				inputVector.x = 0f;
			}
